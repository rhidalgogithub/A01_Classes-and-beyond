...
namespace A01_simple_Class
{
    class A01_main
    {
        ...
        static void ImportData() {
            var importedStudent = new StudentData("Jenny" , "Gonzalez" , "05071995" , 6 , "Active");
            Console.WriteLine("Import() Function called");
            Console.WriteLine($"ImportedStudent : {importedStudent.FirstName}");
        }
...
class StudentData : Member
{
    ...
    public StudentData() {
        Console.WriteLine("Simple Constructor --No parameters-- in action");
    }
    public StudentData( string firstN , string lastN , string dob , int grade , string statusInput ) {
        Console.WriteLine("Constructor in Action -- parameterized");
        FirstName = firstN;
        LastName = lastN;
        DOB = dob;
        Grade = grade;
